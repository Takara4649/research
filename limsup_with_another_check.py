...
# nlogn/root2のグラフをプロット
xs = [i for i in range(1,n)]
sqrts_upper = [math.sqrt(n*math.log(i))/math.sqrt(2) for i in range(1,n)]
plt.plot(xs, sqrts_upper, label="rootn", color="green")
sqrts_lower = [-math.sqrt(n*math.log(i))/math.sqrt(2) for i in range(1,n)]
plt.plot(xs, sqrts_lower, label="rootn", color="green")
...
print("done :)")
plt.savefig("img/rootn_with_limsup_with_simple_random_walk.png")
plt.show()
